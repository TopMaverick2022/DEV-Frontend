...
export function ExplainerPage() {
  ...
  const handleExplain = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to explain. Please try again.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-foreground">Code Explainer</h1>
        <p className="text-muted-foreground mt-1">Paste any code and instantly get a human-readable explanation powered by AI.</p>
      </div>

      <GlassCard>
        <h3 className="font-semibold text-foreground mb-4">Code to Explain</h3>
        <textarea
          value={code}
          onChange={(e) => setCode(e.target.value)}
          placeholder="Paste any code snippet here — any language supported..."
          rows={14}
        ...
        <GlassCard>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-foreground">AI Explanation</h3>
            </div>
