...
import {
  ...
  Image as ImageIcon, FileText, ChevronDown, GitBranch, Maximize2,
} from 'lucide-react'
...
// ── Full PDF export (diagram capture + all data pages) ───────────────────────
async function exportFullPdf(diagramEl: HTMLElement, archData: ArchData, filename = 'architecture') {
  const diagramDataUrl = await toPng(diagramEl, { backgroundColor: '#0f172a', pixelRatio: 2, skipFonts: false })
  ...
  pdf.text(`Services: ${archData.services.length}   •   API Endpoints: ${archData.apis.length}   •   Event Flows: ${archData.events.length}`, margin, margin + 28)
  ...
  pdf.addImage(diagramDataUrl, 'PNG', margin + (contentW - dW) / 2, diagramY, dW, dH)
  ...
  pdf.rect(0, 0, pageW, pageH, 'F')
  let curY = margin + 16
  pdf.setFont('helvetica', 'bold'); pdf.setFontSize(14); pdf.setTextColor(248, 250, 252)
  pdf.text('Service Details', margin, curY); curY += 20
  const colW = (contentW - 12) / 2
  let col = 0, colY = curY
  for (const svc of archData.services) {
    const xOff = margin + col * (colW + 12)
    pdf.setFillColor(30, 41, 59); pdf.setDrawColor(51, 65, 85)
    pdf.roundedRect(xOff, colY, colW, 56, 4, 4, 'FD')
    pdf.setFont('helvetica', 'bold'); pdf.setFontSize(9); pdf.setTextColor(248, 250, 252)
    pdf.text(svc.name, xOff + 8, colY + 13)
    pdf.setFont('helvetica', 'normal'); pdf.setFontSize(7.5); pdf.setTextColor(148, 163, 184)
    pdf.text(pdf.splitTextToSize(svc.description, colW - 16).slice(0, 2), xOff + 8, colY + 24)
    if (svc.database && !['null', 'None', 'none', 'N/A'].includes(svc.database)) {
      pdf.setFillColor(16, 185, 129, 0.15); pdf.setDrawColor(16, 185, 129)
      pdf.roundedRect(xOff + 8, colY + 42, colW - 16, 10, 2, 2, 'FD')
      pdf.setFont('helvetica', 'bold'); pdf.setFontSize(6.5); pdf.setTextColor(16, 185, 129)
    ...
    if (colY + 64 > pageH - margin) {
      col++; colY = curY
      if (col > 1) {
        pdf.addPage(); pdf.setFillColor(15, 23, 42); pdf.rect(0, 0, pageW, pageH, 'F')
        curY = margin + 16; colY = curY; col = 0
      }
    }
  }
  ...
  curY = margin + 16
  pdf.setFont('helvetica', 'bold'); pdf.setFontSize(14); pdf.setTextColor(248, 250, 252)
  pdf.text('API Endpoints', margin, curY); curY += 20
  const colWidths = [60, 180, contentW - 60 - 180 - 8]
  pdf.setFillColor(30, 41, 59); pdf.rect(margin, curY, contentW, 14, 'F')
  pdf.setFont('helvetica', 'bold'); pdf.setFontSize(7); pdf.setTextColor(148, 163, 184)
  let hx = margin + 6
  ;['Method', 'Endpoint', 'Description'].forEach((h, i) => { pdf.text(h, hx, curY + 9); hx += colWidths[i] + 4 })
  curY += 18
  for (const api of archData.apis) {
    if (curY + 18 > pageH - margin) {
      pdf.addPage(); pdf.setFillColor(15, 23, 42); pdf.rect(0, 0, pageW, pageH, 'F'); curY = margin + 10
    }
    pdf.setFillColor(22, 32, 48); pdf.setDrawColor(51, 65, 85); pdf.rect(margin, curY, contentW, 16, 'FD')
    ...
    pdf.setFont('helvetica', 'bold'); pdf.setFontSize(6.5); pdf.setTextColor((rgb >> 16) & 0xff, (rgb >> 8) & 0xff, rgb & 0xff)
    ...
    pdf.setTextColor(148, 163, 184)
    pdf.text(api.description, margin + colWidths[0] + colWidths[1] + 12, curY + 10, { maxWidth: colWidths[2] - 4 })
    curY += 18
  }
  ...
  curY = margin + 16
  pdf.setFont('helvetica', 'bold'); pdf.setFontSize(14); pdf.setTextColor(248, 250, 252)
  pdf.text('Flow Explanation', margin, curY); curY += 20
  archData.events.forEach((evt, idx) => {
    if (curY + 28 > pageH - margin) {
      pdf.addPage(); pdf.setFillColor(15, 23, 42); pdf.rect(0, 0, pageW, pageH, 'F'); curY = margin + 16
    }
    pdf.setFillColor(139, 92, 246, 0.15); pdf.setDrawColor(139, 92, 246)
    pdf.circle(margin + 8, curY + 8, 8, 'FD')
    pdf.setFont('helvetica', 'bold'); pdf.setFontSize(7); pdf.setTextColor(139, 92, 246)
    const label = String(idx + 1)
    pdf.text(label, margin + 8 - pdf.getTextWidth(label) / 2, curY + 11)
    pdf.setFillColor(30, 41, 59); pdf.setDrawColor(51, 65, 85)
    pdf.roundedRect(margin + 20, curY, contentW - 20, 22, 3, 3, 'FD')
    pdf.setFont('helvetica', 'normal'); pdf.setFontSize(8); pdf.setTextColor(200, 210, 230)
    const text = `The ${evt.producer} triggers a ${evt.name}, which is then processed by the ${evt.consumer}.`
    pdf.text(pdf.splitTextToSize(text, contentW - 40), margin + 28, curY + 12)
    curY += 28
  })
...
const EXPORT_OPTS = { backgroundColor: '#0f172a', pixelRatio: 2.5, skipFonts: false }

async function captureFullDiagram(el: HTMLElement, format: ExportFormat): Promise<string> {
  if (format === 'jpeg') return toJpeg(el, { ...EXPORT_OPTS, quality: 0.95 })
  return toPng(el, EXPORT_OPTS)
}

// ── Inner diagram component that can access the ReactFlow context ──────────────
...
function DiagramInner({
  ...
  onConnect, onNodeClick, onPaneClick, canvasRef, fullscreen,
}: DiagramInnerProps) {
  const { fitView } = useReactFlow()

  // Expose fitView so parent can call it before export
  useEffect(() => {
    const el = canvasRef.current
    if (!el) return
    // attach a custom property so the parent can trigger fitView
    ;(el as any).__rfFitView = () => fitView({ padding: 0.15, duration: 300 })
  }, [canvasRef, fitView])
  ...
  return (
    ...
    >
      ...
      <Controls
        style={{ background: 'rgba(15,23,42,0.9)', borderColor: '#334155', borderRadius: 10 }}
        showInteractive={!fullscreen}
      />
    </ReactFlow>
  )
}
...
interface FullscreenModalProps {
  ...
  onConnect: any
  onNodeClick: any
  selectedSvc: ServiceDto | null
  setSelectedSvc: (s: ServiceDto | null) => void
  onClose: () => void
}

function FullscreenModal({
  archData, nodes, edges, onNodesChange, onEdgesChange, onConnect,
  onNodeClick, selectedSvc, setSelectedSvc, onClose,
  ...
  const handleExport = async (fmt: ExportFormat) => {
    ...
    try {
      // Fit entire diagram before capturing
      const fitFn = (fsCanvasRef.current as any).__rfFitView
      if (fitFn) { fitFn(); await new Promise(r => setTimeout(r, 400)) }
      const dataUrl = await captureFullDiagram(fsCanvasRef.current, fmt)
      const a = document.createElement('a')
      a.href = dataUrl; a.download = `architecture-fullscreen.${fmt}`; a.click()
    } finally { setExporting(false) }
  }

  const handleExportPdf = async () => {
    if (!fsCanvasRef.current || !archData) return
    setExporting(true); setExportOpen(false)
    try {
      const fitFn = (fsCanvasRef.current as any).__rfFitView
      if (fitFn) { fitFn(); await new Promise(r => setTimeout(r, 400)) }
      await exportFullPdf(fsCanvasRef.current, archData)
    } finally { setExporting(false) }
  }
  ...
  return (
    ...
    >
      {/* Fullscreen header */}
      <div className="flex items-center gap-3 px-5 py-3 shrink-0 border-b border-slate-700/60"
        style={{ background: 'rgba(15,23,42,0.98)', backdropFilter: 'blur(12px)' }}>
        <div className="p-1.5 rounded-lg bg-primary/10 border border-primary/20">
          <Server className="w-4 h-4 text-primary" />
        </div>
        <span className="font-bold text-sm text-foreground">Architecture — Full View</span>
        <div className="flex items-center gap-2 ml-2 flex-wrap">
          {[
            { label: 'Services', v: archData.services.length },
            { label: 'Endpoints', v: archData.apis.length },
            { label: 'Flows', v: archData.events.length },
          ].map(s => (
            <span key={s.label} className="text-[10px] font-medium px-2 py-0.5 rounded-full border border-border bg-muted/20 text-muted-foreground">
              {s.label}: <strong className="text-foreground">{s.v}</strong>
            </span>
          ))}
        </div>
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <div className="relative">
            <button
              ...
              disabled={exporting}
              className="flex items-center gap-1.5 px-3 py-1.5 border border-border rounded-xl text-sm font-medium hover:bg-muted/50 transition-colors"
            >
            ...
            {exportOpen && (
              <>
                ...
                <div className="absolute right-0 top-full mt-1 z-20 bg-popover border border-border rounded-2xl shadow-2xl min-w-[190px] overflow-hidden">
                  <div className="px-3 py-2 border-b border-border">
                    <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide">Export Full Diagram As</p>
                  </div>
                  ...
                  <button onClick={handleExportPdf}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <span className="block text-[9px] text-muted-foreground">Full report with all details</span>
                    </div>
                  </button>
                </div>
              </>
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-xl border border-border hover:bg-muted/50 transition-colors text-muted-foreground hover:text-foreground"
      ...
      {/* Fullscreen body: diagram + right service panel */}
      <div className="flex flex-1 min-h-0 overflow-hidden">
        ...
        <div className="w-72 shrink-0 border-l border-slate-700/60 overflow-y-auto custom-scrollbar p-4"
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            {selectedSvc && (
              <button onClick={() => setSelectedSvc(null)}
                className="ml-auto text-muted-foreground hover:text-foreground p-0.5 rounded hover:bg-muted/50 transition-colors">
          ...
          {selectedSvc ? (
            <div className="space-y-3">
              ...
              {selectedSvc.database && !['null', 'None', 'none', 'N/A'].includes(selectedSvc.database) && (
                <div className="flex items-center gap-2 px-2.5 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                  ...
                  <span className="text-xs font-semibold text-emerald-400">{selectedSvc.database}</span>
                </div>
              )}
            </div>
          ) : (
            <div className="space-y-2">
              ...
              {archData.services.map((svc, i) => {
                const c = PALETTE[i % PALETTE.length]
                return (
                  <button key={svc.name} onClick={() => setSelectedSvc(svc)}
...
export function ArchitectureGeneratorPage() {
  ...
  const doExport = async (fmt: ExportFormat) => {
    ...
    setExporting(true); setExportOpen(false)
    try {
      // Trigger fitView on the embedded ReactFlow instance via custom property
      const fitFn = (canvasRef.current as any).__rfFitView
      if (fitFn) { fitFn(); await new Promise(r => setTimeout(r, 400)) }
      const dataUrl = await captureFullDiagram(canvasRef.current, fmt)
      const a = document.createElement('a')
      a.href = dataUrl; a.download = `architecture.${fmt}`; a.click()
    } finally { setExporting(false) }
  }
  ...
  const doExportPdf = async () => {
    ...
    try {
      const fitFn = (canvasRef.current as any).__rfFitView
      if (fitFn) { fitFn(); await new Promise(r => setTimeout(r, 400)) }
      await exportFullPdf(canvasRef.current, archData)
    } finally { setExporting(false) }
  }
    ...
    <>
      ...
      {fullscreen && archData && (
        <FullscreenModal
          ...
          onNodesChange={onNodesChange} onEdgesChange={onEdgesChange}
          onConnect={onConnect} onNodeClick={onNodeClick}
          selectedSvc={selectedSvc} setSelectedSvc={setSelectedSvc}
      ...
      <div className="flex flex-col gap-4 pb-8">
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-4">
            ...
            {/* Zone 1: Diagram + Service Details */}
            <div className="flex gap-4" style={{ minHeight: 420 }}>

              {/* Diagram */}
              <div
                ref={canvasRef}
                className="flex-1 min-w-0 rounded-2xl border border-slate-700 overflow-hidden relative"
                style={{ height: 420, background: '#0f172a' }}
              >
                {/* Fullscreen hint button inside diagram */}
                <button
                  onClick={() => setFullscreen(true)}
                  title="Open fullscreen"
                  className="absolute top-3 right-3 z-10 p-1.5 rounded-lg bg-slate-800/80 border border-slate-600/50 text-slate-400 hover:text-white hover:bg-slate-700 transition-all"
                >
                  <Maximize2 className="w-3.5 h-3.5" />
                </button>
            ...
            {/* Zone 2: API Endpoints + Flow Explanation */}
                    ...
                    {flowLines.map((e, i) => (
                      <div key={i} className="flex items-start gap-2.5">
                        ...
                        <div className="text-[11.5px] text-muted-foreground leading-relaxed mt-0.5">
                          The <span className="font-semibold text-foreground">{e.producer}</span> triggers a <span className="font-semibold text-primary">{e.name}</span>, which is then processed by the <span className="font-semibold text-foreground">{e.consumer}</span>.
                        </div>
                      </div>
                    ))}
                  </div>
